...
describe("repository setup review regressions", () => {
  test("keeps Bearing nomination contextual and explicit", () => {
    ...
    expect(pointer).toContain("Repository issue-tracker planning, decisions, or execution");
    expect(pointer).toContain("explicitly invokes another Skill");
    expect(pointer).toContain("compose that Skill behind it");
  ...
  test("creates state and cache namespaces and removes an unselected surface pointer", async () => {
    ...
    const reconciled = await applyRepositoryConfigurationUnit({
      repoRoot,
      packageRoot: process.cwd(),
