...
const contract = `# Issue tracker: GitHub Issues
...
class FixtureGitHubTransport implements GitHubReadTransport {
  readonly requests: GitHubReadRequest[] = [];

  constructor(private readonly fixtures: Readonly<Record<string, FixtureResponse>>) {}
...
const response = (body: unknown, etag: string): GitHubReadResponse => ({
  status: 200,
  headers: { etag },
  body,
});
...
const researchIssue = githubIssue({
  number: 3,
...
const deliveryIssue = githubIssue({
  number: 4,
...
const scopedIncomingIssue = githubIssue({
  number: 5,
  title: "Support a custom-mapped enhancement",
...
const comment = (input: { id: number; issue: number; body: string; author?: string }) => ({
  id: input.id,
...
const createRepository = async (contractBody = contract): Promise<string> => {
  const root = await makeTemporaryDirectory("bearing-github-provider-");
  await writeFixture(root, contractLocator, contractBody);
