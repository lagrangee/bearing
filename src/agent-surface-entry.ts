...
export const BEARING_POINTER =
  "For a new request, load `bearing` for explicit Bearing concepts, reliable direct continuation, or reasonable material planning/governance relevance. Repository issue-tracker planning, decisions, or execution are material governance relevance even when the Human explicitly invokes another Skill; load `bearing` and compose that Skill behind it. Do not load for working directory, generic roadmap words, repository-independent conversation, or ordinary non-governance code/documentation work. This pointer is contextual guidance, not an executable hook or lifecycle preflight. Each requested functional operation validates its lifecycle. Explicit `$bearing` loads the Skill when contextual nomination did not occur.";
export const BEARING_DEVELOPMENT_POINTER =
  "This repository selects the Bearing Development Runtime. For a new request, load the repository-local `$bearing-dev` Skill from `.agents/skills/bearing-dev` for explicit Bearing concepts, reliable direct continuation, or reasonable material planning/governance relevance. Repository issue-tracker planning, decisions, or execution are material governance relevance even when the Human explicitly invokes another Skill; load `$bearing-dev` and compose that Skill behind it. Do not load it for working directory, generic roadmap words, repository-independent conversation, or ordinary non-governance code/documentation work. Run repository-scoped Bearing commands through this checkout's `dist/cli.js`; each command must return a coherent Development Runtime receipt before it can operate. Use `$bearing-dev`, not the public `$bearing`, for this repository. Do not use or fall back to the public Stable Kit, CLI, Skill, or state. Explicit `$bearing-dev` follows this repository-local Development Runtime selection.";
