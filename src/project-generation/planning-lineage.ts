...
type Input = Omit<
  ...
  "providerObservations" | "providerObservationSelections" | "providerDetailEvidences"
> &
  Readonly<{
    providerObservations: ProjectGeneration["providerObservations"];
    providerObservationSelections: ProjectGeneration["providerObservationSelections"];
    boundProviderObservations: ProjectGeneration["providerObservations"];
    boundProviderObservationSelections: ProjectGeneration["providerObservationSelections"];
  }>;
...
const normalizedInput = (input: PlanningLineageBuildInput): Input => {
  ...
  const observations = [
    ...input.providerDetailEvidences.observations,
  ...
  const selections = [
    ...input.providerDetailEvidences.selections,
...
const targetForReference = (input: Input, reference: string, note?: string): RelationTarget => {
  ...
  const unavailableNote =
    subject === undefined
      ? "Stable detail route unavailable for this planning reference in the current Project Read Model generation."
      : "Referenced subject unavailable in the current Project Read Model generation.";
  return {
    reference,
    ...
    return {
      ...
      note:
        candidates.length > 1
          ...
          : (note ??
            "Provider-native route unavailable in the current Project Read Model generation."),
    };
  }
  ...
  return presentRelation("native-work.binding", "Work Binding", "binds to native scope", "one", [
    {
      ...
      availability: available ? "available" : "unavailable",
      ...(scopeSubject === undefined ? {} : { subject: scopeSubject }),
...
const nativeWorkReadingStateFor = (
  ...
  const authoritativeObservation =
    scopeContext.state === "bound"
      ? input.boundProviderObservations.find((candidate) =>
          sameMattNativeScope(candidate.binding, detailObservation.binding),
        )
      : detailObservation;
  ...
  const boundEffort =
    scopeContext.state === "bound"
      ? efforts.find((effort) => effort.id === scopeContext.effortIds[0])
      : undefined;
  const context =
