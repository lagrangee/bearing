...
export const providerDetailEvidenceProjectionSchema = z
  .strictObject({
  ...
  .superRefine((projection, context) => {
    for (const [index, selection] of projection.selections.entries()) {
      if (
        ...
        selection.latestAttempt.intent !== "provider-detail-selection" &&
      ...
      if (
        ...
        context.addIssue({
          ...
          message: "Every provider detail evidence selection must resolve in its exact scope.",
        });
    ...
    for (const [index, observation] of projection.observations.entries()) {
      if (
        ...
        context.addIssue({
          ...
          message: "Every provider detail evidence observation must be selected.",
        });
      }
    }
  });
