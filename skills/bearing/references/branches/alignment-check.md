...
5. **Resolve only with acceptance.** The user may decide now or leave the Check open. An accepted resolution records plain-text `Resolution.Accepted decision`, current UTC `Accepted at`, `Rationale`, and structural `Changed references`, using an explicit empty list when current direction remains. Generate `Accepted at` inside the same successful atomic canonical mutation; a failed resolution writes neither decision nor time. An Authority Adoption created by this resolution cites the Alignment Check and reuses its accepted time through that reference; it never copies or invents an adoption timestamp. Apply only target-scoped Effort, Work Binding, Authority, Asset Registry, citation, and dependent-reference mutations owned here. A transient Scope Review or Governance Disposition may nominate the question but never supplies acceptance or writes the binding. Roadmap- or Gate-exclusive mutations return `awaiting-decision` with the required capability and keep the Check open. Completion: acceptance and owned write scope align.
