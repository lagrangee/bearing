...
4. **Apply owned changes.** After acceptance, re-read the full set and apply only project-wide Effort intent or relation, Authority, Asset Registry, citation, and dependent-reference changes within this capability's ownership. Effort Lifecycle or Conclusion changes remain owned by the Effort Lifecycle branch; the Review supplies its exact accepted decision and is completed atomically by that owner only when it directly authorizes a project-wide lifecycle set. Project Summary-, Roadmap-, or Gate-exclusive changes keep the Review pending and return `awaiting-decision` with the required capability; resume this Review afterward. Completion: no exclusive field is mutated by coordination convenience.
5. **Complete the record.** A Review uses `Type: planning-review`, `ID: planning-review:<slug>`, `Title`, `Status: pending | completed`, `Scope`, ordered `Inputs`, `Input fingerprint`, optional `Citations`, and, when completed, `Resolution` with `Accepted decision`, current UTC `Accepted at`, `Rationale`, and `Changed references`. Generate `Accepted at` inside the same successful atomic canonical mutation; a failed Review writes neither decision nor time. An Authority Adoption created by this Review cites the Review and reuses its accepted time through that reference; it never copies or invents an adoption timestamp. Write every human-readable semantic value under the shared Planning Transaction's Normalized Semantic Text rule; keep IDs, Inputs, fingerprints, and changed references structural. Mark completed only after all accepted changes validate. Completion: record, Source Event Time, and graph are atomically coherent.
...
- Accepted project-wide Effort intent or relation, Authority, Asset Registry, citation, and dependent-reference changes owned here
