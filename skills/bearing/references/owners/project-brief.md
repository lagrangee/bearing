...
1. Inspect Project Context, current Brief, Summary, and accepted terminal planning transitions that
   materially affect orientation. After a transition, complete its canonical post-edit validation
   before synthesizing the Brief, then read the affected Roadmap or Gate from that committed
   generation for any derived state the Brief will state. Do not carry forward pre-transition
   readiness: an unpassed Gate may be `ready-for-review`, `not-ready`, or `unknown`. Give every
   Brief statement accepted support.
