...
The Local PRD materializes the existing Matt Kit `to-spec` document contract. Keep one
`# <Spec title>` and a plain `Status: ready-for-agent` in the preamble when publishing accepted
work for implementation. Preserve the seven sections below and fill them from the accepted
intent and decisions. Draft work uses `Status: draft`; a replaced Spec uses `Status: superseded`.

- `## Problem Statement`: the user's problem.
- `## Solution`: the accepted outcome from the user's perspective.
- `## User Stories`: numbered actor, desired behavior, and benefit statements.
- `## Implementation Decisions`: the agreed implementation decisions.
- `## Testing Decisions`: the agreed public behavior seams and checks.
- `## Out of Scope`: the accepted exclusions.
- `## Further Notes`: relevant additional context.

This is the repository's physical native document format. Ordinary authorized work uses it
without claiming a user-only Skill invocation.
